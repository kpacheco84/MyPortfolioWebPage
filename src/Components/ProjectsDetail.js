...
const  ProjectDetails = () => {
    ...
    const renderCard = (card,index)=>{
        ...
        <Card className='column2box' style={{ width: '18rem',height:'400px'}} key = {index} imgsrc = {card.image}>
      <Card.Body>
        <Card.Title style={{paddingBottom:'10px'}}>{card.title}</Card.Title>
        <Card.Text>
          <ul > { card.tech.map(tech => 
                < li style={{align:'left',textAlign:'left'}}  key={index} name = 'Row' id ='projRow'>{tech}</li>
        ...
        <Button style={{backgroundColor:'#ff4d4d',border:'0px',marginTop:'20px'}}>Description</Button>
      </Card.Body>
